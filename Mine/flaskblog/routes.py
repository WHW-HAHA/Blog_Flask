...
def save_picture(form_picture):
        random_hex = secrets.token_hex(8) # to return a 8 digits token key
        _, f_ext = os.path.splitext(form_picture.filename)
        picture_fn = random_hex + f_ext
        picture_path =os.path.join(app.root_path, 'static/profile_pics', picture_fn)
        output_size = (125, 125) # 125^2 pixels
...
def account():
    ...
    if form.validate_on_submit():
        if form.picture.data:
            ...
            current_user.image_file = picture_file # replace the default user profile picture with new picture
...
@app.rute("/post/new", methods = ['GET', 'Post'])
@login_required
def new_post():
    form = PostForm()
    if form.validate_on_submit():
...
@app.route("/post/<int:post_id>/delete", method = ['Post']) # delete is always a post action and not get action
...
def send_reset_email(user):
    ...
    msg.body = f" To reset your passwrd, visit the following link:\
                {url_for('reset_token', token = token, _external=True)}\
                If you did not make this request then simply ignore this eamail and no change will be made."
    mail.send(msg)
...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        flash('An email has been sent to reset your current passowords.', 'info')
